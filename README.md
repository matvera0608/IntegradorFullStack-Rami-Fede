# Sistema de Hotel con REACT + VITE
Este proyecto es un sistema completo para la gestión de un hotel, incluyendo:

1. Información del hotel y tipos de habitaciones.
2. Gestión de reservas.
3. Catálogo de buffet con pedidos a la habitación.
4. Panel de administración para usuarios, reservas, habitaciones y pedidos.
5. Contiene una base de datos llamada Hotel.
6. Además se hizo con el fin de mostrar nuestra primer API Rest.

Pero una diferencia sutil es que ese sistema simple estará incluido en react.
Este proyecto es similar a lo que hicimos con backend pero esta vez será con versión de react
